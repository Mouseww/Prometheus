...
with tempfile.TemporaryDirectory(prefix="prometheus-readonly-e2e-") as temporary:
    temporary_path = Path(temporary)
    workspace = temporary_path / "workspace"
    workspace.mkdir()
    shutil.copyfile(ROOT / "README.md", workspace / "README.md")
    ...
    try:
        ...
        subprocess.run(
            [sys.executable, str(ROOT / "scripts" / "e2e_readonly_tools.py")],
            cwd=ROOT,
            check=True,
        )
