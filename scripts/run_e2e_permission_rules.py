...
with tempfile.TemporaryDirectory(prefix="prometheus-permissions-e2e-") as temporary:
    ...
    try:
        ...
        test_env["PROMETHEUS_E2E_WORKSPACE"] = str(workspace)
        subprocess.run(
            [sys.executable, str(ROOT / "scripts" / "e2e_permission_rules.py")],
            cwd=ROOT,
            env=test_env,
            check=True,
        )
