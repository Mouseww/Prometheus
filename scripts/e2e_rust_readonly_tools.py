"""HTTP-level E2E: openai_compatible fixture + Rust tool loop (readonly)."""
...
def main() -> int:
    workspace = ROOT / ".prometheus-e2e-rust-tools"
    workspace.mkdir(exist_ok=True)
    (workspace / "README.md").write_text("# Prometheus\nRust tool runtime E2E\n", encoding="utf-8")
    ...
    env["PROMETHEUS_PORT"] = "4318"
    env["PROMETHEUS_HOST"] = "127.0.0.1"
    env["PROMETHEUS_MASTER_KEY"] = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="

    fixture = subprocess.Popen([sys.executable, str(FIXTURE)], cwd=str(ROOT), text=True)
    server = subprocess.Popen([str(SERVER_BIN)], cwd=str(ROOT), env=env, text=True)
    try:
        wait_http("http://127.0.0.1:4320/health")
        wait_http("http://127.0.0.1:4318/api/health")

        _, provider_body = http_json(
            "POST",
            "http://127.0.0.1:4318/api/providers",
            {
        ...
        _, agent_body = http_json(
            "POST",
            "http://127.0.0.1:4318/api/agents",
            {
                "name": "Runtime verifier",
                "description": "tools",
                "systemPrompt": "Answer with verifiable evidence.",
                "providerId": provider_body["provider"]["id"],
                "model": "fixture-model",
            },
            expected=201,
        )
        _, session_body = http_json(
            ...
            {"title": "Rust tools E2E"},
            expected=201,
        )
        session_id = session_body["session"]["id"]
        http_json(
            "POST",
            f"http://127.0.0.1:4318/api/sessions/{session_id}/events",
            {
                "eventId": str(uuid.uuid4()),
                "type": "message.user",
                "actor": {"kind": "user", "id": "user", "label": "You"},
                "payload": {"text": "Inspect the repository with tools."},
            },
            expected=201,
        )
        _, run_body = http_json(
            "POST",
            f"http://127.0.0.1:4318/api/sessions/{session_id}/runs",
            {"agentId": agent_body["agent"]["id"]},
            expected=201,
        )
