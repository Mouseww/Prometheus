...
with tempfile.TemporaryDirectory(prefix="prometheus-team-worktree-e2e-") as temporary:
    temporary_path = Path(temporary)
    workspace = temporary_path / "workspace"
    worktree_root = temporary_path / "worktrees"
    source = workspace / "src"
    source.mkdir(parents=True)
    (workspace / "README.md").write_text("# Team worktree fixture\n", encoding="utf-8")
    (source / "shared.txt").write_text("base shared\n", encoding="utf-8")
    git(workspace, "init")
    git(workspace, "config", "core.autocrlf", "false")
    git(workspace, "config", "user.email", "prometheus-e2e@example.com")
    git(workspace, "config", "user.name", "Prometheus E2E")
    git(workspace, "add", ".")
    ...
    try:
        ...
        server_env.update({
            ...
            "PROMETHEUS_WORKTREE_ROOT": str(worktree_root),
        ...
        test_env.update({
            "PROMETHEUS_E2E_WORKSPACE": str(workspace),
            "PROMETHEUS_E2E_WORKTREE_ROOT": str(worktree_root),
        })
        subprocess.run(
            [sys.executable, str(ROOT / "scripts" / "e2e_team_worktree.py")],
            cwd=ROOT,
            env=test_env,
            check=True,
        )
        ...
        print(fixture_log_path.read_text(encoding="utf-8", errors="replace"), file=sys.stderr)
